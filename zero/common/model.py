...
def build_data():
    ...
    tokenized_dataset = dataset.map(partial(tokenize, mode=TOKEN_MODE), batched=True, num_proc=16, remove_columns='text')
...
class ModelFromHF(torch.nn.Module):
    def __init__(self, config):
        ...
        self.module.resize_token_embeddings(VOCAB_SIZE)
...
def build_model():
    config = GPT2Config(n_positions=SEQ_LEN, n_embd=HIDDEN_SIZE, n_layer=DEPTH, n_head=NUM_HEADS)
...
def build_optimizer(params=None):
